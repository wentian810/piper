...
def resolve_local_dataset_root(repo_id: str, root: Path | None) -> Path:
    if root is not None:
    ...
    local_dataset_path = Path(f"/home/bbncf305/lerobot513/DP/lerobot_piper3/datasets/{repo_id.split('/')[-1]}")
...
def handle_delete_episodes(cfg: EditDatasetConfig) -> None:
    ...
    local_root = resolve_local_dataset_root(cfg.repo_id, Path(cfg.root) if cfg.root else None)
    dataset = LeRobotDataset(cfg.repo_id, root=local_root)
    output_repo_id, output_dir = get_output_path(cfg.repo_id, cfg.new_repo_id, local_root)
