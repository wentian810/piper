...
class LeRobotDatasetMetadata:
    def __init__(
        ...
        try:
            ...
            # In local/offline workflows we may already have the dataset on disk.
            # Avoid contacting the Hugging Face Hub unless the local root itself is missing.
            if not self.root.exists() and is_valid_version(self.revision):
            ...
            if not (self.root / "meta" / "info.json").exists():
                raise FileNotFoundError(
                    f"Local dataset metadata not found at {self.root / 'meta' / 'info.json'}"
                )
            self.load_metadata()
    ...
    def pull_from_repo(
        ...
        """Populate local files from the Hub only when the requested paths are missing locally."""
        if allow_patterns == "meta/":
            required_meta_files = [
            ...
            if any(path.exists() for path in required_meta_files):
                return
    ...
    def __getitem__(self, idx) -> dict:
        ...
        if len(self.meta.video_keys) > 0:
            ...
            try:
                ...
                logging.warning(
                    "Skipping dataset index %s (episode %s) due to video timestamp mismatch: %s",
                    idx,
                    ep_idx,
                    exc,
                )
